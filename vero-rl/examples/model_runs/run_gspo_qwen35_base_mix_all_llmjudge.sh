#!/bin/bash
# Train Qwen3.5-9B-Base with GSPO + LLM Judge
set -x
set -o pipefail

DATE_TIME="$(date +%Y%m%d_%H%M%S)"
TRAINER_EXPERIMENT_SUFFIX="qwen3_5_9b_base_mix_all"
...
python3 -m verl.trainer.main_ppo \
  ...
  --config-name='gspo_qwen35' \
  actor_rollout_ref.model.path="Qwen/Qwen3.5-9B-Base-nothinkchattemplate"
