...
def get_fsdp_wrap_policy(module, config=None, is_lora=False):
    ...
    if min_num_params > 0:
        ...
        # [qunzhong] patch for molmo2
        # the reason is the structure of molmo2 is different from the other models
        for layer_class in fsdp_transformer_layer_cls_to_wrap:
            transformer_cls = get_module_class_from_name(module, layer_class)
            if transformer_cls is None:
                print(f"Transformer layer class '{layer_class}' not found in model, skipping.")
            else:
                transformer_cls_to_wrap.add(transformer_cls)
                print(f"Transformer layer class '{layer_class}' found in model. Wrapped")
...
def apply_fsdp2(model, fsdp_kwargs, config):
    ...
    # [qunzhong] patch for molmo2 only. Avoid children wrapped in parent
    if "Molmo2VisionBlock" in fsdp_transformer_layer_cls_to_wrap and "ViTMultiHeadDotProductAttention" in fsdp_transformer_layer_cls_to_wrap:
        fsdp_transformer_layer_cls_to_wrap = [c for c in fsdp_transformer_layer_cls_to_wrap if c != "ViTMultiHeadDotProductAttention"]
