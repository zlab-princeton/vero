...
def aligning_dim1(values):
    """
    Align the dimension 1 of the values to the maximum dimension 1.
    """
    max_dim1 = max(v.shape[1] for v in values)
    padded_values = []
    for v in values:
        if v.shape[1] < max_dim1:
            padding = torch.zeros(v.shape[0], max_dim1 - v.shape[1], v.shape[2], v.shape[3], v.shape[4], dtype=v.dtype, device=v.device)
...
def extract_multi_modal_inputs(
    ...
    for inputs in selected_batch_data:
        ...
        if "batch_num_images" in inputs and "image_sizes" in inputs:
            # print("bee logic detected")
            has_image_sizes = True # bee logic
    ...
    for key, values in multi_modal_inputs_collected.items():
        ...
        elif has_image_sizes: # bee logic
            if key in ["batch_num_images", "image_sizes"]:
                multi_modal_inputs[key] = torch.cat(values, dim=0)
            else:
                # print("aligning dim1")
                # for i, v in enumerate(values):
                #     print(f"value[{i}] shape: {v.shape}")
                values = aligning_dim1(values)
                # for i, v in enumerate(values):
                #     print(f"value[{i}] shape after aligning: {v.shape}")
                multi_modal_inputs[key] = torch.cat(values, dim=0)
        else:
            multi_modal_inputs[key] = torch.cat(values, dim=0)
