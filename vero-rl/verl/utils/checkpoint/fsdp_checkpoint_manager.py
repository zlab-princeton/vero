...
def _patch_device_mesh_compat():
    """Patch DeviceMesh to reconstruct torch-2.10 private attrs from old public ones.

    Torch 2.10+ restructured DeviceMesh internals:
      - ``device_type`` → ``_device_type`` (private + @property)
      - ``mesh_dim_names`` → ``_mesh_dim_names`` (private + @property)
      - NEW ``_layout`` = ``_MeshLayout(mesh.size(), mesh.stride())``
      - NEW ``_rank_map`` = ``mesh.flatten()``
      - ``_flatten_mesh_list`` → ``_flatten_rank_map``

    Checkpoints saved with older torch have the old attrs in ``__dict__``
    (``device_type``, ``mesh``, ``mesh_dim_names``). When unpickled under
    torch 2.10, these DeviceMesh objects lack the new private attrs,
    causing ``AttributeError`` during ``load_state_dict`` → ``copy_()``.

    This patch adds ``__getattr__`` that **reconstructs** the new attrs
    from the old ones (caching them for subsequent access).
    """
    try:
        from torch.distributed.device_mesh import DeviceMesh

        if getattr(DeviceMesh, "_verl_compat_patched", False):
            return
        ...
        def _compat_getattr(self, name):
            ...
            if name == "_mesh_dim_names":
                val = d.get("mesh_dim_names")
                if val is not None:
                    val = tuple(val)
                d["_mesh_dim_names"] = val
                return val

            if name == "_device_type":
                val = d.get("device_type", "cuda")
                d["_device_type"] = val
                return val
            ...
            if name == "_layout":
                ...
                if mesh is not None:
                    ...
                    mesh_t = mesh.detach().to(dtype=torch.int).contiguous()
                    layout = _MeshLayout(mesh_t.size(), mesh_t.stride())
                    d["_layout"] = layout
                    return layout
                raise AttributeError(
                    f"Cannot reconstruct '_layout': old DeviceMesh has no 'mesh' tensor"
                )

            if name == "_rank_map":
                mesh = d.get("mesh")
                if mesh is not None:
                    rank_map = mesh.detach().to(dtype=torch.int).flatten()
                    d["_rank_map"] = rank_map
                    return rank_map
                raise AttributeError(
                    f"Cannot reconstruct '_rank_map': old DeviceMesh has no 'mesh' tensor"
                )

            if name == "_flatten_rank_map":
                # Old torch had _flatten_mesh_list; new torch uses _flatten_rank_map
                old_val = d.get("_flatten_mesh_list")
                if old_val is not None:
                    d["_flatten_rank_map"] = old_val
                    return old_val
                # Reconstruct from _rank_map
                rank_map = getattr(self, "_rank_map", None)
                if rank_map is not None:
                    val = tuple(rank_map.tolist())
                    d["_flatten_rank_map"] = val
                    return val
                raise AttributeError(
                    f"Cannot reconstruct '_flatten_rank_map'"
                )
        ...
        DeviceMesh.__getattr__ = _compat_getattr
        DeviceMesh._verl_compat_patched = True
    except ImportError:
        pass
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        # Ensure cross-version DeviceMesh compatibility before unpickling
        _patch_device_mesh_compat()
