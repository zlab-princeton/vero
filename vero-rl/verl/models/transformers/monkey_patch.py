...
def patch_forward_with_backends(
    ...
    if model.config.model_type in ["qwen2_5_vl", "qwen2_vl"]:
        ...
        forward_with_torch_backend_function = forward_with_torch_backend
        forward_with_triton_backend_function = forward_with_triton_backend
    elif model.config.model_type == "Bee":
        from verl.models.transformers.bee import forward_with_torch_backend, forward_with_triton_backend
        ...
        from verl.models.transformers.llava_onevision import forward_with_torch_backend, forward_with_triton_backend
...
def apply_monkey_patch(
    ...
    if model.config.model_type in ["qwen2_5_vl", "qwen2_vl"]:
        ...
        # Patch GatedDeltaNet forward to support packed sequences (use_remove_padding=True).
        # The patched forward passes seq_idx to causal_conv1d and cu_seqlens to
        # chunk_gated_delta_rule so linear attention layers respect sequence boundaries.
        ...
        print(f"Monkey patch Qwen3_5GatedDeltaNet.forward for packed sequence support")

        # Patch full attention forward to fix position_ids for packed mode.
        # transformers 5.2.0 bug: Qwen3_5TextModel passes mrope_position_ids (3, bsz, seqlen)
        # to decoder layers, but flash attention needs text_position_ids (bsz, seqlen) for
        # varlen sequence boundary detection (prepare_fa2_from_position_ids).
        # Guard: only patch once to avoid infinite recursion if apply_monkey_patch runs twice.
        if not hasattr(Qwen3_5Attention, "_original_forward"):
            Qwen3_5Attention._original_forward = Qwen3_5Attention.forward
            Qwen3_5Attention.forward = qwen3_5_attn_forward
            print(f"Monkey patch Qwen3_5Attention.forward for packed mode position_ids fix")

        # Step 2: Ulysses SP is NOT supported for Qwen3.5 hybrid attention.
        # Only full attention layers (25%) would be sequence-parallelized; the 75%
        # linear attention (GatedDeltaNet) layers are not intercepted by the Ulysses
        # flash attention hook, causing silent correctness divergence.
        if ulysses_sp_size > 1:
            raise ValueError(
                f"Ulysses sequence parallelism (ulysses_sp_size={ulysses_sp_size}) is not supported "
                f"for Qwen3.5 models. The hybrid attention architecture (GatedDeltaNet + Full Attention) "
                f"requires ulysses_sp_size=1. Set actor_rollout_ref.model.ulysses_sp_size=1."
            )
        ...
        from verl.models.transformers.glm4v import forward_with_normal_backend, glm4v_base_forward, glm4v_text_model_forward

        Glm4vModel.forward = glm4v_base_forward
        Glm4vForConditionalGeneration.forward = forward_with_normal_backend
        print(f"Monkey patch {model.__class__.__name__} model forward")
        
        # Patch the language model (Glm4vTextModel) forward function
        Glm4vTextModel.forward = glm4v_text_model_forward
        print(f"Monkey patch Glm4vTextModel.forward for custom position_ids handling")
        ...
        from verl.models.transformers.bee import forward_with_normal_backend
        
        # step 1.
        # Patch the model's forward function
        model.__class__.forward = forward_with_normal_backend
        print(f"Monkey patch {model.__class__.__name__} (Bee) model forward")

    elif model.config.model_type == "molmo2":
        from verl.models.transformers.molmo2 import forward_with_normal_backend
    
        model.__class__.forward = forward_with_normal_backend
        print(f"Monkey patch {model.__class__.__name__} (Molmo2) model forward")
        # Ulysses SP input slicing would need the Molmo2TextModel class if needed
        # For now, we rely on the standard flash attention handling

    elif model.config.model_type == "llavaonevision1_5":
        # LLaVA-OV 1.5 is a trust_remote_code model — get classes from the loaded module
        from verl.models.transformers.llava_onevision import (
            forward_with_normal_backend,
            llavaonevision15_attn_forward,
            llavaonevision15_base_forward,
        )

        # Step 1: patch model to support image-text mixed data (dummy visual for FSDP)
        LLaVAOV15Model = getattr(module, "LLaVAOneVision1_5_Model")
        LLaVAOV15Model.forward = llavaonevision15_base_forward
        model.__class__.forward = forward_with_normal_backend
        print(f"Monkey patch {model.__class__.__name__} (LLaVA-OneVision 1.5) model forward")

        # Step 2: patch attention to support packed mode + Ulysses SP
        if use_remove_padding or ulysses_sp_size > 1:
            LLaVAOV15FlashAttn = getattr(module, "LLaVAOneVision1_5_FlashAttention2")
            LLaVAOV15FlashAttn.forward = llavaonevision15_attn_forward
            print(f"Monkey patch {model.__class__.__name__} attention layer")

        # Step 3: patch input for multimodal sequence parallelism
        if ulysses_sp_size > 1:
            LLaVAOV15TextModel = getattr(module, "LLaVAOneVision1_5_TextModel")
            patch_vlm_for_ulysses_input_slicing(LLaVAOV15TextModel)
