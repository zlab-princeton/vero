...
def parse_args():
    ...
    base_op_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a base Molmo HF directory containing modeling_molmo2.py and related custom code.",
    )
...
class ModelMergerConfig:
    ...
    Args:
        ...
        molmo_base_path (Optional[str]): Path to a base Molmo HF directory with modeling code. Defaults to None.
    ...
    molmo_base_path: Optional[str] = None
...
def generate_config_from_args(args: argparse.Namespace) -> ModelMergerConfig:
    common_config_args = {
        ...
        "molmo_base_path": args.molmo_base_path,
...
class BaseModelMerger(ABC):
    ...
    def __init__(self, config: ModelMergerConfig):
        ...
        self.raw_config = self._load_raw_config(self.hf_model_config_path)
        self.is_molmo = self._is_molmo_model(self.raw_config)
        self._validate_molmo_setup_if_needed()
        self.model_config = AutoConfig.from_pretrained(
            self.hf_model_config_path, trust_remote_code=self.config.trust_remote_code
        )
        self._patch_molmo_auto_map_if_needed()

    @staticmethod
    def _load_raw_config(hf_model_config_path: str) -> dict:
        config_json_path = os.path.join(hf_model_config_path, "config.json")
    ...
    @staticmethod
    def _is_molmo_model(raw_config: dict) -> bool:
        model_type = str(raw_config.get("model_type", "")).lower()
        architectures = raw_config.get("architectures", [])
        architecture_name = architectures[0] if architectures else ""
        return model_type.startswith("molmo") or "Molmo" in architecture_name

    def _validate_molmo_setup_if_needed(self):
        if not self.is_molmo:
            return

        if not self.config.trust_remote_code:
            raise ValueError("Molmo checkpoint merge requires --trust-remote-code.")

        if not self.config.molmo_base_path:
            raise ValueError(
                "Molmo checkpoint detected. Please provide --molmo-base-path pointing to a base Molmo HF directory."
            )

        required_files = ("modeling_molmo2.py", "configuration_molmo2.py", "config.json")
        missing_files = [
            os.path.join(self.config.molmo_base_path, file_name)
            for file_name in required_files
            if not os.path.exists(os.path.join(self.config.molmo_base_path, file_name))
        ]
        if missing_files:
            raise FileNotFoundError(
                "Molmo base path is missing required files:\n" + "\n".join(missing_files)
            )

    def _patch_molmo_auto_map_if_needed(self):
        if not self.is_molmo:
            return

        if not hasattr(self.model_config, "auto_map") or self.model_config.auto_map is None:
            self.model_config.auto_map = {}

        self.model_config.auto_map["AutoModelForImageTextToText"] = (
            f"{self.config.molmo_base_path}--modeling_molmo2.Molmo2ForConditionalGeneration"
        )

    def get_transformers_auto_model_class(self):
        architecture_name = self.model_config.architectures[0]
        has_remote_code = hasattr(self.model_config, "auto_map") and any(
            architecture_name in val for val in self.model_config.auto_map.values()
        )
        if has_remote_code:
            auto_class = next(
                k for k, v in self.model_config.auto_map.items() if architecture_name in v
            )
            match auto_class:
                ...
                case "AutoModelForImageTextToText":
                    if _AutoModelForImageTextToText is not None:
