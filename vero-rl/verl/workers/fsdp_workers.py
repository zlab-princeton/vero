...
# TEMP_MOLMO2_RESUME_LR_BEGIN
# Temporary override for Molmo2 resume runs; remove after experiments conclude.
MOLMO2_RESUME_FORCED_LR = 5e-6
# TEMP_MOLMO2_RESUME_LR_END
...
def get_vl_model_vision_tower(vl_model_instance):
    ...
    Supports `visual` (e.g., Qwen VL family), `vision_backbone` (e.g., Molmo2),
    and `vision_tower` (e.g., Bee).
    """
    if hasattr(vl_model_instance, "model") and hasattr(vl_model_instance.model, "visual"):
        # transformers >= 4.52.0
        return vl_model_instance.model.visual
    elif hasattr(vl_model_instance, "model") and hasattr(vl_model_instance.model, "vision_tower"):
        return vl_model_instance.model.vision_tower
    elif hasattr(vl_model_instance, "model") and hasattr(vl_model_instance.model, "vision_backbone"):
        return vl_model_instance.model.vision_backbone
    elif hasattr(vl_model_instance, "visual"):
        # transformers < 4.52.0
        return vl_model_instance.visual
    elif hasattr(vl_model_instance, "vision_tower"):
        return vl_model_instance.vision_tower
    elif hasattr(vl_model_instance, "vision_backbone"):
        return vl_model_instance.vision_backbone
    return None
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        # Disable Multi-Token Prediction for Qwen3.5 (not used in RL training)
        if getattr(actor_model_config, "model_type", None) in ("qwen3_5", "qwen3_5_moe"):
            if hasattr(actor_model_config, "text_config") and getattr(actor_model_config.text_config, "mtp_num_hidden_layers", 0) > 0:
                actor_model_config.text_config.mtp_num_hidden_layers = 0
                if self.rank == 0:
                    print("[Qwen3.5] Disabled MTP heads (mtp_num_hidden_layers=0) for RL training")
            if self.rank == 0 and hasattr(actor_model_config, "text_config"):
                tc = actor_model_config.text_config
                ...
                if layer_types:
                    ...
                    print(f"[Qwen3.5] Hybrid: {linear_count} linear (GatedDeltaNet) + {full_count} full attention layers")
        ...
        # NOTE: accelerate 1.12.0 + transformers 5.2.0 crashes with meta_tensor=True
        # due to _is_hf_initialized kwarg in Parameter.__new__() (big_modeling.py:135).
        # Force use_meta_tensor=False for Qwen3.5 until accelerate is fixed.
        use_meta = not actor_model_config.tie_word_embeddings
        if getattr(actor_model_config, "model_type", None) in ("qwen3_5", "qwen3_5_moe"):
            use_meta = False
        init_context = get_init_weight_context_manager(
            use_meta_tensor=use_meta, mesh=self.device_mesh
        )
    ...
    def load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
        ...
        self._maybe_force_molmo2_resume_lr(local_path=local_path)
    ...
    # TEMP_MOLMO2_RESUME_LR_BEGIN
    def _maybe_force_molmo2_resume_lr(self, local_path: str) -> None:
        """Force a fixed LR for resumed Molmo2 actor checkpoints only."""
        if not self._is_actor or self.actor_optimizer is None:
            return

        config_path = os.path.join(local_path, "huggingface", "config.json")
        try:
            ...
            if self.rank == 0:
                logger.warning("Failed to inspect checkpoint model_type at %s: %s", config_path, exc)
            return

        if model_type != "molmo2":
            return

        forced_lr = MOLMO2_RESUME_FORCED_LR

        for param_group in self.actor_optimizer.param_groups:
            param_group["lr"] = forced_lr
            if "initial_lr" in param_group:
                param_group["initial_lr"] = forced_lr

        if self.actor_lr_scheduler is not None:
            if hasattr(self.actor_lr_scheduler, "base_lrs"):
                base_lrs = getattr(self.actor_lr_scheduler, "base_lrs")
                if isinstance(base_lrs, list):
                    self.actor_lr_scheduler.base_lrs = [forced_lr for _ in base_lrs]
            if hasattr(self.actor_lr_scheduler, "_last_lr"):
                self.actor_lr_scheduler._last_lr = [forced_lr] * len(self.actor_optimizer.param_groups)

        if self.rank == 0:
            logger.warning(
                "Applied temporary Molmo2 resume LR override to %.8g from checkpoint %s",
                forced_lr,
                local_path,
            )
    # TEMP_MOLMO2_RESUME_LR_END
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        # Disable Multi-Token Prediction for Qwen3.5 (not used in RL training)
        if getattr(critic_model_config, "model_type", None) in ("qwen3_5", "qwen3_5_moe"):
            if hasattr(critic_model_config, "text_config") and getattr(critic_model_config.text_config, "mtp_num_hidden_layers", 0) > 0:
                critic_model_config.text_config.mtp_num_hidden_layers = 0

        use_meta = not critic_model_config.tie_word_embeddings
        if getattr(critic_model_config, "model_type", None) in ("qwen3_5", "qwen3_5_moe"):
            use_meta = False
        init_context = get_init_weight_context_manager(
            use_meta_tensor=use_meta, mesh=self.device_mesh
