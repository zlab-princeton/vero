...
# patch: for dev version compatibility
def filter_unsupported_llm_kwargs(kwargs: dict) -> dict:
    """
    Filter out unsupported kwargs for LLM.__init__() based on the actual signature.
    """
    try:
        ...
        if filtered_out:
            logger.warning(
                f"Filtered out unsupported LLM parameters: {filtered_out}"
            )
        return filtered_kwargs
    except Exception as e:
        logger.warning(f"Failed to filter LLM kwargs: {e}. Using original kwargs.")
        return kwargs
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        # patch: Filter out unsupported parameters for this vLLM version
        llm_kwargs =  dict(
            model=model_path,
        ...
        llm_kwargs = filter_unsupported_llm_kwargs(llm_kwargs)
    ...
    async def release(self):
        ...
        if hasattr(self.inference_engine, "reset_mm_cache"):
