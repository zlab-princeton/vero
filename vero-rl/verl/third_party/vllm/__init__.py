...
# [qunzhong] Patch, Check if this is a development version
dev_ver = False
if package_version and (".dev" in package_version or "+g" in package_version):
    dev_ver = True
...
elif dev_ver or vs.parse(package_version) >= vs.parse("0.7.0"):
    vllm_version = package_version
    if dev_ver or vs.parse(package_version) >= vs.parse("0.8.5"):
        VLLM_SLEEP_LEVEL = 2
    from vllm import LLM
    from vllm.distributed import parallel_state
else:
