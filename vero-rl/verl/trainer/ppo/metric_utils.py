    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                valid_entries = [(idx, val) for idx, val in enumerate(var_vals) if val is not None]
                if not valid_entries:
                    continue

                filtered_var_vals = [val for _, val in valid_entries]
                if isinstance(filtered_var_vals[0], str):
                    continue
                    ...
                    for n in ns:
                        ...
                        if var2vals.get("pred", None) is not None:
                            vote_data = [
                                ...
                                for idx, val in valid_entries
                                if var2vals["pred"][idx] is not None
                            ]
                            if len(vote_data) >= n:
    ...
    for data_source, var2metric2uid_vals in data_src2var2metric2uid_vals.items():
        for var_name, metric2uid_vals in var2metric2uid_vals.items():
            for metric_name, uid_vals in metric2uid_vals.items():
                filtered_uid_vals = [val for val in uid_vals if val is not None]
                if not filtered_uid_vals:
                    continue
                data_src2var2metric2val[data_source][var_name][metric_name] = np.mean(filtered_uid_vals)
